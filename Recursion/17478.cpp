...
string func1(int n) {
  if (n == 0) return ""; 
  return func1(n - 1) + "____";
}

void func2(int n) {
  cout << func1(n) << "\"재귀함수가 뭔가요?\"\n";
  if (n == N) cout << func1(n) << "\"재귀함수는 자기 자신을 호출하는 함수라네\"\n";
  else {
    cout << func1(n) << "\"잘 들어보게. 옛날옛날 한 산 꼭대기에 이세상 모든 지식을 통달한 선인이 있었어.\n"
         << func1(n) << "마을 사람들은 모두 그 선인에게 수많은 질문을 했고, 모두 지혜롭게 대답해 주었지.\n"
         << func1(n) << "그의 답은 대부분 옳았다고 하네. 그런데 어느 날, 그 선인에게 한 선비가 찾아와서 물었어.\"\n";
    func2(n + 1);
  }
  cout << func1(n) << "라고 답변하였지.\n";
}
