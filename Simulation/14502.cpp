...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      cin >> board[i][j];
      if (board[i][j] == 0) cand.push_back({i,j});
      else if (board[i][j] == 2) virus.push_back({i,j});
    }
  }
  vector <bool> mask(cand.size(),1);
  fill(mask.begin(),mask.begin() + 3, 0);
  do {
    int nBoard[9][9];
    for (int i = 0; i < n; i++) copy(board[i], board[i] + m, nBoard[i]);
    for (int i = 0, cnt = 0; cnt < 3; i++) {
      if (mask[i]) continue;
      nBoard[cand[i].X][cand[i].Y] = 1;
      cnt++;
    }
    queue <pair<int, int>> Q;
    for (auto v : virus) Q.push(v);
    int safe = cand.size() - 3;
    while (!Q.empty()) {
